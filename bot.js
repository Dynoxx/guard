...
Constants.DefaultOptions.ws.properties.$browser = `Discord Android` // yada Discord Android 

client.on("ready",()=>{
  client.user.setActivity(`Developer:> forcen`);
...
const emmmmbed = new Discord.MessageEmbed()
  ...
  .addField(
    `Dynox | Teşekkürler`,
    `**Selamlar, Ben Dynox (Forcen Bot'un Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`
  )
  .addField(
    `Dynox | Prefix`,
    `**Dynox Botun Prefixi(ön eki) = \`+\`\n\n Değiştirebilmek için \`+prefix\` Yazabilirsiniz.**`
  )
  .addField(
    `Dynox | Nasıl Kullanılır?`,
    `**Dynox botun tüm özelliklerinden yararlanabilmek için sadece \`+yardım\` yazmanız yeterlidir.**`
  )
  .addField(
    `Dynox | Linkler`,
    `**Sohbet Kanalına +davet Yazmanız Yeterlidir**`
  )
  .setFooter(`Dynox | Gelişmiş Türkçe Bot | 2021`)
  .setTimestamp();
      ...
      try {
        if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
          ...
          let embed = new Discord.MessageEmbed()
            .setColor(0xffa300)
            .setFooter("Dynox Küfür Sistemi", client.user.avatarURL())
            .setAuthor(
...
client.on("guildMemberAdd", member => {
  ...
  let embed = new Discord.MessageEmbed()
    .setDescription(
      "> <a:Dynox_hypes:794222389584068618> **Sunucuya yeni katılan** **" +
        member.user.username +
        "** **Kullanıcısına** <@&" +
        rol +
        "> **Rolü verildi** <a:Dynox_tiks:798275047047168041>"
    )
...
client.on("ready", async () => {
  let botVoiceChannel = client.channels.cache.get("82358668588639855x");
